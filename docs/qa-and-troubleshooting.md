...
先用完整订阅配置建立基线，再逐步最小化。最终方案把 Mac 客户端作为首选代理；Ubuntu 备用只有满足地区门槛后才启用，降低重复实现和漂移风险。

### Python 探测 Auth 返回 `530`，但 curl 返回 `200`

这可能不是节点故障，而是上游对默认 User-Agent 的差异化处理。本次复现为：curl 默认 User-Agent 和 `clash.meta` 均返回 `200`，`Python-urllib/3.11` 返回 `530`，同一路径的 Codex 端点仍返回 `403`。

排查时固定代理、URL、请求方法和请求体，只替换 User-Agent；确认差异后，在探测器中显式使用兼容 User-Agent。不要因为测试客户端被拦截就淘汰节点。回归测试必须覆盖这个请求头。

### `sudo -n -l` 显示命令可运行，实际操作仍要求密码

命令匹配成功不等于命中了 `NOPASSWD`。本次权限清单允许 restart，但 stop/start 仍需密码，预检查却返回成功。

正确做法是只添加探测事务所需的精确命令，使用 `visudo -cf` 校验，然后实际执行一次 `sudo -n /usr/bin/systemctl stop ...` 和 `start ...`。脚本不再以 `sudo -n -l` 作为权限真值；真实操作失败时会报告缺失的具体 action/service，并由恢复逻辑保护生产配置。

### 客户端显示很多低延迟节点，真实 GPT 探测却多数失败

客户端的通用延迟地址只能证明某个测试站点可达，不能证明 OpenAI Device Auth 与 Codex 同时可用。当前受控线上测试中，51 个节点只有 3 个通过两个真实端点，且只有韩国达到“至少 2 节点”的地区门槛。

如果无法得到 2 个地区、每区至少 2 个可用节点，就停止备用组上线，保留 Mac Shadowrocket 正式路径并恢复原生产配置。不要为了凑数量放宽真实端点标准。
